...
(let* (
  ...
  (let ((bf (copy-tree TEST-1)))
    (setq bf (appliquerRegleBf '*r1* bf))
    (setq bf (appliquerRegleBf '*r1* bf))
  ...
  (let ((but (copy-tree TEST-2)))
    (setq but (appliquerRegleBut '*r1* but))
    (setq but (appliquerRegleBut '*r1* but))
  ...
  (let ((bf (copy-tree TEST-2)) resultat)
    (setq resultat (chainageArriere bf *br* '((oeufsBlancs 10)) nil))
    (if (not resultat)
      (print "Test 9-1 reussi")
      (print "Test 9-1 échoué : bf non différent de TEST-1")
    )
    (setq resultat (chainageArriere bf *br* '((oeufsBlancs 3) (oeufsJaunes 4)) nil))
    (if resultat
